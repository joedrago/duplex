...
export class EmbeddedSubtitles {
    ...
    async _makeAssSink(track) {
        ...
        // letterboxes the canvas to sit exactly over the picture. JASSUB is
        // built to bind to a <video> and read its dimensions; we have none (the
        // WebCodecs player draws to its own canvas), so seed the size it would
        // otherwise pull off the video — without this, resize() dereferences
        // the absent `_video` and throws "Cannot read properties of undefined".
        jassub._videoWidth = this.videoWidth
        jassub._videoHeight = this.videoHeight
